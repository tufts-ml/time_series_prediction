...
def get_best_model_file(saved_model_files_aka):
    ...
    for i, training_file in enumerate(training_files):
        ...
        loss_per_fit_list.append(train_perf_df['val_loss'].values[-1])
...
#         print('Adding missing values mask as features...')
        ...
        for states in ['5', '10', '20', '30', '50']:
            for lamb in ['1000', '5000', '10000', '50000', '100000']:
                saved_model_files_aka = os.path.join(clf_models_dir, "pchmm*n_states=%s-*lamb=%s.csv"%(states, lamb))
                ...
                print('Evaluating PCHMM with n_states=%s and lambda=%s with model %s'%(n_states, lamb, best_model_file))
                ...
                print('lambda : %s, ROC-AUC : %.2f'%(lamb, np.percentile(roc_auc_np, 50)))
    ...
    perf_csv = os.path.join(args.output_dir, 'pchmm_perlambda_performance.csv')
    print('Saving PCHMM per-lambda performance to %s'%perf_csv)
