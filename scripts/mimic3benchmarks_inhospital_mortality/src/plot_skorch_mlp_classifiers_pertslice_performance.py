...
if __name__ == '__main__':
    ...
    perf_csvs = glob.glob(os.path.join(args.performance_csv_dir,'skorch_mlp_pertslice_performance.csv'), recursive=True)
    ...
    perf_measures = ['roc_auc', 'average_precision', 'recall_score', 'precision_score', 'balanced_accuracy']
    fontsize=18
    prefix = 'skorch_mlp_'
    for x in tslice_grouped_list:
        for perf_measure in perf_measures:
            ...
            if x==neg_tslices_list:
                ...
                fig_aka='%sperf_%s_neg_hours.png'%(prefix, perf_measure)
            elif x==perc_tslices_list:
                axs.set_xlabel('% Data Observed', fontsize=fontsize)
                fig_aka='%sperf_%s_perc_hours.png'%(prefix, perf_measure)
            elif x==pos_tslices_list:
                axs.set_xlabel('Hours From Admission', fontsize=fontsize)
                fig_aka='%sperf_%s_pos_hours.png'%(prefix, perf_measure)
            ...
            if perf_measure == 'roc_auc':
                ...
                axs.set_ylim([0.1, 0.9])
            elif perf_measure == 'balanced_accuracy':
