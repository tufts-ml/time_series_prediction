...
def plot_best_model_training_plots(best_model_file):
    ...
    f.savefig('fully-supervised-best-model-training-plots.png')
...
#                 for lamb in ['1000', '5000', '10000', '50000', '100000']:
            saved_model_files_aka = os.path.join(clf_models_dir, "fully-supervised-pchmm*perc_labelled=%s-*.csv"%(perc_labelled))
            ...
            print('Evaluating PCHMM with perc_labelled =%s with model %s'%(perc_labelled, best_model_file))
            ...
            print('perc_labelled = %s, ROC-AUC : %.2f'%(perc_labelled, np.percentile(roc_auc_np, 50)))
    ...
    #         print('Median balanced accuracy score : %.3f'%np.median(balanced_accuracy_np))
            ...
            for prctile in prctile_vals:
                ...
                row_dict['n_states'] = n_states

                perf_df = perf_df.append(row_dict, ignore_index=True)      

    
    perf_csv = os.path.join(args.output_dir, 'fully_supervised_pchmm_performance.csv')
    print('Saving fully-supervised PCHMM performance to %s'%perf_csv)
