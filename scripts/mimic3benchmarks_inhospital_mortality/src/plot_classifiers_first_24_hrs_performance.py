...
if __name__ == '__main__':
    ...
    final_perf_csvs = []
    chosen_models = ['pchmm_', 'rnn_']
    
    for r in chosen_models:
        for i in perf_csvs:
            if r in i:
                final_perf_csvs.append(i)
    del perf_csvs
    perf_csvs = final_perf_csvs
    ...
    neg_tslices_list = [i for i in tslices_list if '-' in str(i)]
    perc_tslices_list = [i for i in tslices_list if '%' in str(i)]
    pos_tslices_list = []
    ...
    for x in tslice_grouped_list:
        for perf_measure in perf_measures:
            ...
            if x==neg_tslices_list:
                ...
                axs.set_xticks(x)
                axs.set_xticklabels([str(i) for i in x])
