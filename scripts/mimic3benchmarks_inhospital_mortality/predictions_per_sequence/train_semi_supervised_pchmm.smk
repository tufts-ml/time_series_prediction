...
rule train_and_evaluate_classifier_many_hyperparams:
    input:
        [os.path.join(RESULTS_FEAT_PER_TSTEP_PATH,"semi-supervised-pchmm-lr={lr}-seed={seed}-batch_size={batch_size}-perc_labelled={perc_labelled}-n_states={n_states}-lamb={lamb}.csv").format(lr=lr, seed=seed, batch_size=batch_size, perc_labelled=perc_labelled, n_states=n_states, lamb=lamb) for lr in config['lr'] for seed in config['seed'] for batch_size in config['batch_size'] for perc_labelled in config['perc_labelled'] for n_states in config['n_states'] for lamb in config['lamb']]
...
rule train_and_evaluate_classifier:
    ...
    params:
        ...
        fn_prefix="semi-supervised-pchmm-lr={lr}-seed={seed}-batch_size={batch_size}-perc_labelled={perc_labelled}-n_states={n_states}-lamb={lamb}"
    
    output:
        os.path.join(RESULTS_FEAT_PER_TSTEP_PATH, "semi-supervised-pchmm-lr={lr}-seed={seed}-batch_size={batch_size}-perc_labelled={perc_labelled}-n_states={n_states}-lamb={lamb}.csv")
