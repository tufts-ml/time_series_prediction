...
'''
Compare Multiple batch sizes Surrogate Loss for LR
----------------------------------------------------------------------------------------------------------------------------
>> snakemake --cores 1 --snakefile evaluate_skorch_lr_with_dynamic_input_output_multiple_batch_sizes.smk evaluate_performance
'''
...
RESULTS_COLLAPSED_FEAT_DYNAMIC_INPUT_OUTPUT_PATH="/cluster/tufts/hugheslab/prath01/results/mimic3/collapsed_features_dynamic_input_output/skorch_logistic_regression_gamma_fp=7"


rule evaluate_performance:
    input:
        script=os.path.join(os.path.abspath('../'), "src", "evaluate_skorch_lr_surrogate_loss_multiple_batch_sizes.py")
