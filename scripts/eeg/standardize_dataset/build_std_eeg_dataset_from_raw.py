...
def standardize_eeg_data(input_csv_path='data.csv'):
    ...
    tidy_df : Pandas DataFrame
        subject_id, sequence_id, timestep, eeg_signal, seizure_label
    '''
    data = pd.read_csv(input_csv_path)
    data['id'] = data[data.columns[0]]
    data['subject_id'] = data['id'].str.extract(r'X\d*\.(.*)', expand=False)
    data['sequence_id'] = data['id'].str.extract(r'X(\d*)\..*',
    ...
    tidy_df = pd.wide_to_long(data, stubnames='X', i=['subject_id', 'sequence_id'],
                              j='timestep')
    ...
    tidy_df = tidy_df[['subject_id', 'sequence_id', 'timestep', 'eeg_signal',
                       'seizure_binary_label', 'category_label']].copy()
    tidy_df.sort_values(['subject_id', 'sequence_id', 'timestep'], inplace=True)
...
if __name__ == '__main__':
    ...
    # Write to csv file
    if output_all_csv_path:
        tidy_df.to_csv(output_all_csv_path, index=False, header=True)

    static_df = pd.pivot_table(
        tidy_df[['subject_id', 'sequence_id', 'seizure_binary_label', 'category_label']],
        index=['subject_id', 'sequence_id'])
    static_df.to_csv(output_labels_static_csv_path, index=True)

    tidy_ts_df = tidy_df[['subject_id', 'sequence_id', 'timestep', 'eeg_signal']]
