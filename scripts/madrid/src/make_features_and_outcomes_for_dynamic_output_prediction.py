...
if __name__ == '__main__':
    ...
    for p in pbar(range(n_iters)):
        ...
        # get the time bins
        t_bins = np.arange(0, t_end+bin_hrs, bin_hrs)
        ...
        if len(t_bin_inds)>0:
            for bin_ind in range(1, max(t_bin_inds)+1):
                ...
                if ((curr_adm_outcome==1)&(curr_bin_t>=t_end)):
    ...
    outcomes_df = pd.DataFrame(outcomes_np, columns=id_cols+[time_col] + [outcome_col] + ['stay_length'])
...
#     outcomes_df = pd.merge(keep_ids_df, outcomes_df, on=id_cols, how='inner')
    ...
    features_csv_filename = os.path.join(args.output_dir, 'features_T_hrs_before_clinical_deterioration.csv.gz')
    ...
    outcomes_csv_filename = os.path.join(args.output_dir, 'outcomes_T_hrs_before_clinical_deterioration.csv.gz')
