...
if __name__ == '__main__':
    ...
    # save the per feature means, mins and maxs for imputation and normalization of test set
    print('Getting per feature means, min and max from padded sequences for imputation and normalization of test set')
    per_feature_means, per_feature_mins, per_feature_maxs = get_per_feature_estimates_from_padded_sequences(X_train_tensor, seq_lens_N)
        
    per_feature_means_file = os.path.join(args.output_dir, 'per_feature_means.npy')
    np.save(per_feature_means_file, per_feature_means)
    
    per_feature_mins_file = os.path.join(args.output_dir, 'per_feature_mins.npy')
    np.save(per_feature_mins_file, per_feature_mins)
    
    per_feature_maxs_file = os.path.join(args.output_dir, 'per_feature_maxs.npy')
    np.save(per_feature_maxs_file, per_feature_maxs)
    
    # Normalize train and test data
    print('Normalizing data...')
    X_train_tensor = normalize_data(X_train_tensor, seq_lens_N, per_feature_mins, per_feature_maxs)
...
#              EarlyStopping(monitor='valid_loss', patience=20, threshold=0.00001, threshold_mode='abs', lower_is_better=True),
