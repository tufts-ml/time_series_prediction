...
#     y_train_valid = np.concatenate([y_train, y_valid])
    ...
    x_train_df['window_end_timestamp'] = pd.to_datetime(x_train_df['admission_timestamp'])+pd.to_timedelta(x_train_df['stop'], 'h')
    x_valid_df['window_end_timestamp'] = pd.to_datetime(x_valid_df['admission_timestamp'])+pd.to_timedelta(x_valid_df['stop'], 'h')
    x_test_df['window_end_timestamp'] = pd.to_datetime(x_test_df['admission_timestamp'])+pd.to_timedelta(x_test_df['stop'], 'h')
    ...
    models_dict = {
        'logistic regression' : {'dirname':'skorch_logistic_regression', 
                 ...
                 'lightGBM' : {'dirname': 'lightGBM',
                                   'model_constructor' : None,
                                   'prefix' : '*min_samples_per_leaf=1024-max_leaves=8-n_estimators=100-frac_features_for_clf=0.33-frac_training_samples_per_tree=0.33',
                                    'model_color' : 'g',
                                    'model_marker' : 'o'},
