...
if __name__ == '__main__':
    ...
    parser.add_argument('--include_medications', default='True', type=str,
                       help='flag to add/not add medications to features')
    ...
    _,labs_data_dict,_,vitals_data_dict, _, demographics_data_dict, _,medications_data_dict, _, _ = get_preprocessed_data(args.preproc_data_dir)
...
#     clf_models_dir = os.path.join(args.clf_models_dir, 'current_best_model')
    ...
    # get training set normalization estimates
    ...
    for p, tslice in enumerate(tslices_list):
        ...
        medications_df = pd.read_csv(os.path.join(tslice_folder, 'medications_before_icu_filtered_%s_hours.csv.gz'%tslice))
        ...
        test_features_df = get_time_since_last_observed_features(test_features_df, id_cols, time_col, non_medication_feature_cols)
