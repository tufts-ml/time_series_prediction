...
>> snakemake --cores 1 --snakefile skorch_mlp_dynamic_custom_times.smk
'''

# Default environment variables
# Can override with local env variables

configfile:"skorch_mlp_sl.json"
...
RESULTS_COLLAPSED_FEAT_DYNAMIC_INPUT_OUTPUT_PATH = os.path.join(RESULTS_COLLAPSED_FEAT_DYNAMIC_INPUT_OUTPUT_PATH, 'skorch_mlp', 'CustomTimes_10_6')
...
rule train_and_evaluate_classifier_many_hyperparams:
    input:
        [os.path.join(RESULTS_COLLAPSED_FEAT_DYNAMIC_INPUT_OUTPUT_PATH, "skorch_mlp_lr={lr}-weight_decay={weight_decay}-batch_size={batch_size}-scoring={scoring}-seed={seed}-lamb={lamb}-initialization_gain={initialization_gain}-n_hiddens={n_hiddens}-min_precision={min_precision}-n_layers={n_layers}-warm_start={warm_start}-features_to_include={features_to_include}_perf.csv").format(lr=lr, weight_decay=weight_decay, batch_size=batch_size, scoring=scoring, seed=seed, lamb=lamb, initialization_gain=initialization_gain, n_hiddens=n_hiddens, n_layers=n_layers, min_precision=min_precision, warm_start=warm_start, features_to_include=features_to_include) for lr in config['lr'] for weight_decay in config['weight_decay'] for batch_size in config['batch_size'] for scoring in config['scoring'] for seed in config['seed'] for lamb in config['lamb'] for initialization_gain in config['initialization_gain'] for n_hiddens in config['n_hiddens'] for n_layers in config['n_layers'] for min_precision in config['min_precision'] for warm_start in config['warm_start'] for features_to_include in config['features_to_include']]
...
rule train_and_evaluate_classifier:
    ...
    params:
        ...
        fn_prefix="skorch_mlp_lr={lr}-weight_decay={weight_decay}-batch_size={batch_size}-scoring={scoring}-seed={seed}-lamb={lamb}-initialization_gain={initialization_gain}-n_hiddens={n_hiddens}-min_precision={min_precision}-n_layers={n_layers}-warm_start={warm_start}-features_to_include={features_to_include}"
    ...
    shell:
        ...
        python -u {input.script} \
            ...
            --n_splits 1 \
            --min_precision {wildcards.min_precision} \
