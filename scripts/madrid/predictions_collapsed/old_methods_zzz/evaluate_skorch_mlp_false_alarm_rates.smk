'''
Get the alarm rates of a single trained classifier for clinical deterioration at multiple patient-stay-slices

Usage : Evaluate the performance of different classifiers (LR, RF, MEWS) on multiple patient-stay-slices 
----------------------------------------------------------------------------------------------------------------------------
>> snakemake --cores 1 --snakefile evaluate_skorch_mlp_false_alarm_rates.smk evaluate_performance
...
COLLAPSED_FEAT_PER_TIMESLICE_CLF_MODELS_PATH = os.path.join(COLLAPSED_FEAT_PER_TIMESLICE_CLF_MODELS_PATH, 'skorch_mlp')

rule evaluate_performance:
    input:
        script=os.path.join(os.path.abspath('../'), "src", "evaluate_skorch_mlp_false_alarm_rates.py")
