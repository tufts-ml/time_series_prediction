...
----------------------------------------------------------------------------------------------------------------------------
>> snakemake --cores 1 --snakefile evaluate_lr_rf_mlp_mews_pertslice_performance.smk evaluate_performance
...
rule evaluate_performance:
    input:
        script=os.path.join(os.path.abspath('../'), "src", "evaluate_lr_rf_mlp_mews_pertslice_performance.py")
