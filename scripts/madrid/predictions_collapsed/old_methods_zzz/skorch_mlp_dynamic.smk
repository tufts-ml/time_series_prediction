...
rule train_and_evaluate_classifier:
    ...
    params:
        ...
        fn_prefix="skorch_mlp_lr={lr}-weight_decay={weight_decay}-batch_size={batch_size}-scoring={scoring}-seed={seed}-lamb={lamb}-initialization_gain={initialization_gain}-n_hiddens={n_hiddens}-n_layers={n_layers}-warm_start={warm_start}"

    output:
        os.path.join(RESULTS_COLLAPSED_FEAT_DYNAMIC_INPUT_OUTPUT_PATH, "skorch_mlp_lr={lr}-weight_decay={weight_decay}-batch_size={batch_size}-scoring={scoring}-seed={seed}-lamb={lamb}-initialization_gain={initialization_gain}-n_hiddens={n_hiddens}-n_layers={n_layers}-warm_start={warm_start}_perf.csv")
