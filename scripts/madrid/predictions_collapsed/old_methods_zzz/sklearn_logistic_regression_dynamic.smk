...
>> snakemake --cores 1 --snakefile sklearn_logistic_regression_dynamic.smk
'''

# Default environment variables
# Can override with local env variables

configfile:"sklearn_logistic_regression_bce.json"
...
rule train_and_evaluate_classifier_many_hyperparams:
    input:
        [os.path.join(RESULTS_COLLAPSED_FEAT_DYNAMIC_INPUT_OUTPUT_PATH, "sklearn_logistic_regression_weight_decay={weight_decay}_perf.csv").format(weight_decay=weight_decay) for weight_decay in config['weight_decay']]

rule train_and_evaluate_classifier:
    input:
        script=os.path.join(PROJECT_REPO_DIR, 'src', 'SklearnLogisticRegression', 'main.py'),
    ...
    params:
        ...
        fn_prefix="sklearn_logistic_regression_weight_decay={weight_decay}"

    output:
        os.path.join(RESULTS_COLLAPSED_FEAT_DYNAMIC_INPUT_OUTPUT_PATH, "sklearn_logistic_regression_weight_decay={weight_decay}_perf.csv")
