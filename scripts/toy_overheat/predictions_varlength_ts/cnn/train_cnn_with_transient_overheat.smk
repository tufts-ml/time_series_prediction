...
rule train_and_evaluate_classifier:
    ...
    params:
        ...
        fn_prefix="cnn-transient-conv_layers={n_conv_layers}-filters={n_filters}-kernel_size={kernel_size}-stride={stride}-pool={pool_size}-dense={dense_units}-lr={lr}-dropout={dropout}-weight_decay={weight_decay}-batch_size={batch_size}-seed={seed}"
    
    output:
        os.path.join(RESULTS_FEAT_PER_TSTEP_PATH, "cnn-transient-conv_layers={n_conv_layers}-filters={n_filters}-kernel_size={kernel_size}-stride={stride}-pool={pool_size}-dense={dense_units}-lr={lr}-dropout={dropout}-weight_decay={weight_decay}-batch_size={batch_size}-seed={seed}.csv")
