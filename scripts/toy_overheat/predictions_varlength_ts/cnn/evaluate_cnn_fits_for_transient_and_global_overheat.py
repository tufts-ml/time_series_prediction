...
if __name__ == '__main__':
    ...
    overheats = ['transient', 'global']
    
    for overheat in overheats:
        all_fits_csvs = glob.glob(os.path.join(args.fits_dir, "cnn-%s-*.csv"%overheat))
        losses_per_fit_list = []
        auc_per_fit_list = []
    ...
    #     best_fit_ind = np.argmin(losses_per_fit_list)
        ...
        print('Best CNN fit for %s overheat : %s'%(overheat, best_fit_csv))
        print('Best CNN AUC on test set for %s overheat : %.2f'%(overheat, test_auc))
