...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='rnn fits evaluation for toy overheat')
    parser.add_argument('--fits_dir', type=str, required=True)
    args = parser.parse_args()
        
    #go through all the saved loss plots and choose the best one
    for overheat in ['global', 'transient']:
        all_fits_jsons = glob.glob(os.path.join(args.fits_dir, "rnn-%s-*history.json"%overheat))
    ...
    #     best_fit_ind = np.argmin(losses_per_fit_list)
        ...
        print('Best RNN fit for %s overheat : %s'%(overheat, best_fit_csv))
        print('Best RNN AUC on test set for %s overheat : %.2f'%(overheat, test_auc))
