...
#         role 'input' and type 'integer' or 'number' (as specified in the data
#         dictionary) are normalized. Scaling is based on each column's
...
for x in data_dict['fields']:
    col = x['name']
    if (x['role'] == 'input' and x['type'] in ('integer', 'number')):
