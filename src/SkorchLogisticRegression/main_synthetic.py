...
if __name__ == '__main__':
    ...
    x_valid = pd.read_csv(x_valid_csv).values.astype(np.float32)
...
#             logistic_clf.module_.linear_transform_layer.bias.data = torch.tensor(thr_grid[gg]).double()
            curr_thr_y_preds = logistic_clf.predict_proba(x_valid_transformed)[:,1]>=thr_grid[gg] 
...
#             recall_train = recall_score_G[best_ind]
        ...
        if args.warm_start == 'true':
            ...
            logistic_init = SkorchLogisticRegression(n_features=x_train.shape[1],
                                                l2_penalty_weights=args.weight_decay,
    #                                             l2_penalty_bias=args.weight_decay,
                                                lr=args.lr,
                                                batch_size=args.batch_size, 
...
#             logistic.initialize()
#             #init from near ground truth
#             logistic.module_.linear_transform_layer.weight.data[0,:2] = torch.tensor([-1.0, 0.8])
#             logistic.module_.linear_transform_layer.bias.data[0] = torch.tensor(-1.05) 

#             #init from uniform(-1, 1)
#             r1 = -1.5
#             r2 = 1.5
#             logistic.module_.linear_transform_layer.weight.data[0] = (r1-r2)*torch.rand(D) + r2
#             logistic.module_.linear_transform_layer.bias.data[0] = (r1-r2)*torch.rand(1) + r2
            
#             #init from normal(0, 2)
#             init_mean = -1
#             init_std = 1
#             logistic.module_.linear_transform_layer.weight.data[0] = init_std*torch.rand(D) + init_mean
#             logistic.module_.linear_transform_layer.bias.data[0] = init_std*torch.rand(1) + init_mean
