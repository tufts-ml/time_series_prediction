...
#                                                        lr_scheduler,
        ...
        buffer = torch.randn(D).double()
        logistic.module_.linear_transform_layer.weight.data = logistic_init_clf.module_.linear_transform_layer.weight.data + buffer[:D-1]
        logistic.module_.linear_transform_layer.bias.data = logistic_init_clf.module_.linear_transform_layer.bias.data + buffer[D-1]
...
#         y_test = y_test[:, np.newaxis]
        ...
        print_st_tr = "Training performance minimizing %s loss at threshold %.5f : N_train %d | Precision %.3f | Recall %.3f | TP %5d | FP %5d | TN %5d | FN %5d"%(args.scoring, thr, len(x_train), precision_train, recall_train, TP_train, FP_train, TN_train, FN_train)
        print_st_te = "Test performance minimizing %s loss at threshold %.5f : N_test %d | Precision %.3f | Recall %.3f | TP %5d | FP %5d | TN %5d | FN %5d"%(args.scoring, thr, len(x_test), precision_test, recall_test,TP_test, FP_test, TN_test, FN_test) 
