...
#         # y (output)
#         self.w_hy = torch.nn.Parameter(torch.Tensor(hidden_size, output_size))

#         # bias
#         self.b_dg_x = torch.nn.Parameter(torch.Tensor(hidden_size))
...
#             gamma_x = torch.exp(-1* torch.nn.functional.relu( self.w_dg_x * d + self.b_dg_x))
#             gamma_h = torch.exp(-1* torch.nn.functional.relu( self.w_dg_h * d + self.b_dg_h))
...
#             step_output = torch.matmul(h, self.w_hy)  + self.b_y
#             step_output = torch.sigmoid(step_output)
#             output_tensor[:,timestep,:] = step_output
#             hidden_tensor[:,timestep,:] = h
