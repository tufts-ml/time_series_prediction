...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='sklearn randomforest')
    ...
    thr_list = [0.5]
    ## start training
    min_samples_leaf_clf = np.minimum(args.min_samples_leaf, len(x_train))
    max_features_clf = np.maximum(1, int(np.round(args.frac_features_for_clf * len(feature_cols))))
    
    print('Training Random Forest Classifier with a minimum of %d samples per leaf and %d features used per split...'% (min_samples_leaf_clf, max_features_clf))
    clf = RandomForestClassifier(class_weight='balanced', criterion='gini', max_depth=None, max_features=max_features_clf,
                                   min_samples_leaf=min_samples_leaf_clf, n_estimators=25, verbose=10)
    
    
    rf_clf = clf.fit(x_train_transformed, y_train)
    ...
    y_train_valid_proba_vals = rf_clf.predict_proba(x_train_valid_transformed)
    ...
    for gg, thr in enumerate(thr_grid): 
        curr_thr_y_preds = rf_clf.predict_proba(x_train_valid_transformed)[:,1]>=thr_grid[gg] 
    ...
    for thr in thr_list : 
        y_train_pred_probas = rf_clf.predict_proba(x_train_transformed)[:,1]
        y_train_pred = y_train_pred_probas>=thr
        precision_train = precision_score(y_train, y_train_pred)
        recall_train = recall_score(y_train, y_train_pred)
        
        y_valid_pred_probas = rf_clf.predict_proba(x_valid_transformed)[:,1]
        ...
        y_test_pred_probas = rf_clf.predict_proba(x_test_transformed)[:,1]
    ...
    dump(rf_clf, open(model_file, 'wb'))
