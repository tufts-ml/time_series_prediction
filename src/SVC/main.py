...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='sklearn SVC')
    ...
    thr_list = [0.5]
    ## start training
    print('Training Supprt Vector Classifier with l2 penalty %s and kernel %s...'% (str(args.l2_penalty), args.kernel))
#     clf = SVC(C=args.l2_penalty, kernel=args.kernel, gamma='scale', 
#               class_weight='balanced', probability=True, verbose=100)
    
    clf_init = LinearSVC(penalty='l2', loss='squared_hinge', tol=0.001, C=args.l2_penalty, class_weight='balanced',
                         max_iter=15000, verbose=100)
    clf = CalibratedClassifierCV(clf_init, cv=5)
    svc_clf = clf.fit(x_train_transformed, y_train)
    ...
    y_train_valid_proba_vals = svc_clf.predict_proba(x_train_valid_transformed)
    ...
    for gg, thr in enumerate(thr_grid): 
        curr_thr_y_preds = svc_clf.predict_proba(x_train_valid_transformed)[:,1]>=thr_grid[gg] 
    ...
    for thr in thr_list : 
        y_train_pred_probas = svc_clf.predict_proba(x_train_transformed)[:,1]
        y_train_pred = y_train_pred_probas>=thr
        precision_train = precision_score(y_train, y_train_pred)
        recall_train = recall_score(y_train, y_train_pred)
        
        y_valid_pred_probas = svc_clf.predict_proba(x_valid_transformed)[:,1]
        ...
        y_test_pred_probas = svc_clf.predict_proba(x_test_transformed)[:,1]
    ...
    dump(svc_clf, open(model_file, 'wb'))
    print('saved SVC model to : %s'%model_file)
