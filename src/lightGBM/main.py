...
#     y_train_valid_pred_probas = lgb_clf.predict_proba(x_train_valid_transformed)
    ...
    for thr in thr_list : 
        y_train_pred_probas = lgb_clf.predict_proba(x_train_transformed)[:,1]
        y_train_pred = y_train_pred_probas>=thr
        precision_train = precision_score(y_train, y_train_pred)
        recall_train = recall_score(y_train, y_train_pred)
        
        y_valid_pred_probas = lgb_clf.predict_proba(x_valid_transformed)[:,1]
        ...
        y_test_pred_probas = lgb_clf.predict_proba(x_test_transformed)[:,1]
    ...
    dump(lgb_clf, open(model_file, 'wb'))
