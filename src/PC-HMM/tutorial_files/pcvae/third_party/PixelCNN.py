...
from tensorflow_probability.python.layers import weight_norm, DistributionLambda
...
class PixelCNN(distribution.Distribution):
    ...
    def get_dist_helper(self, params):
        num_channels = self.event_shape[-1]
        if num_channels == 1:
            component_logits, locs, scales, value = params
        else:
            ...
            component_logits, locs, scales, coeffs, value = params
    ...
    def get_dist(self, value, conditional_input=None, training=None):

        transformed_value = Lambda(lambda x: ((2. * (x - self._low) /
                                               (self._high - self._low)) - 1.))(value)
        # transformed_value = value
        inputs = [transformed_value, conditional_input]
        params = self.network._network(inputs, training=training)
        print('params', type(params), params)
        # return DistributionLambda(lambda p: tfd.Independent(tfd.Normal(scale=1., loc=p[1][:, :, :, 0])))(params)
        return DistributionLambda(self.get_dist_helper)(params + [value])
            ...
            for _ in range(self._num_resnet):
                ...
                for stack in ['vertical', 'horizontal']:
                    ...
                    if conditional_input is not None:
                        ...
                        stride = h_projection.shape[-2] // x.shape[-2]
                        h_projection_pooled = tf.keras.layers.AveragePooling2D(stride, stride)(h_projection)
                        x = tf.keras.layers.add([x, h_projection_pooled], dtype=dtype)
...
def _build_and_apply_h_projection(h, num_filters, dtype):
    ...
    h_projection = tf.keras.layers.Conv2D(
        2 * num_filters, 1, kernel_initializer='random_normal', dtype=dtype)(h)
    return h_projection
