...
#                                                       random_state=213, shuffle=False)
    ...
    X_train = np.delete(X_train, unlabelled_inds_tr, axis=0)
    y_train = np.delete(y_train, unlabelled_inds_tr, axis=0)  
    
    rnd_state = np.random.RandomState(state_id)
    n_unlabelled_va = int((1-(args.perc_labelled)/100)*N_va)
    unlabelled_inds_va = rnd_state.permutation(N_va)[:n_unlabelled_va]
    X_val = np.delete(X_val, unlabelled_inds_va, axis=0)
    y_val = np.delete(y_val, unlabelled_inds_va, axis=0)  
    
    rnd_state = np.random.RandomState(state_id)
    n_unlabelled_te = int((1-(args.perc_labelled)/100)*N_te)
    unlabelled_inds_te = rnd_state.permutation(N_te)[:n_unlabelled_te]
    X_test = np.delete(X_test, unlabelled_inds_te, axis=0)
    y_test = np.delete(y_test, unlabelled_inds_te, axis=0)      
